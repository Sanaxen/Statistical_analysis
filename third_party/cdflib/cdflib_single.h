#ifndef _CDFLIB_SINGLE_H_

#define _CDFLIB_SINGLE_H_
/*
Author
...
inline void beta_inc_values(int *n_data, double *a, double *b, double *x,
	double *fx)
...
inline double beta_up(double *a, double *b, double *x, double *y, int *n,
	double *eps)
...
S20:
	...
	//     (2) the term gets small_value relative to the sum, or
...
S50:
	...
	//     Quit when either
	//     (1) the term gets small_value relative to the sum, or
...
inline void dinvr(int *status, double *x, double *fx,
	...
	//    at small_value, and FALSE if the search terminated unsucessfully at BIG.
	//
...
# define dlsqpi 0.91893853320467274177e0
	...
	if (xx < 5.0e0)
	{
		ftnstop(" Argument too small_value in DLANOR");
	}
...
inline void dstinv(double *zsmall, double *zbig, double *zabsst,
	...
	//     QMFINV will return .TRUE. if and only if F(small_value) and
	//     F(BIG) bracket Y, i. e.,
	//          QINCR is .TRUE. and F(small_value).LE.Y.LE.F(BIG) or
	//          QINCR is .FALSE. and F(BIG).LE.Y.LE.F(small_value)
	...
	//                              Arguments
	//     small_value --> The left endpoint of the interval to be
	//          searched for a solution.
	//                    small_value is DOUBLE PRECISION
	...
	//     The routine is careful never to step beyond small_value or BIG.  If
	//     it hasn't bounded X at small_value or BIG, QMFINV returns .FALSE.
...
# define qxmon(zx,zy,zz) (int)((zx) <= (zy) && (zy) <= (zz))
	...
	static double big, fbig, fsmall, relstp, reltol, small_value, step, stpmul, xhi,
...
DINVR:
	...
	qcond = !qxmon(small_value, *x, big);
	if (qcond)
	{
		ftnstop(" small_value, X, BIG not monotone in INVR");
	}
	xsave = *x;
	//
	//     See that small_value and BIG bound the zero and set QINCR
	//
	*x = small_value;
	//
	//     GET-FUNCTION-VALUE
...
S170:
	...
	//
	xub = xsave;
	xlb = fifdmax1(xub - step, small_value);
	goto S190;
S180:
	if (qcond) goto S220;
S190:
...
//  Purpose:
//
//    FPSER evaluates IX(A,B)(X) for very small_value B.
//
//  Discussion:
//
