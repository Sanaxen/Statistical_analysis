...
void test1()
{
	...
	Lasso_Regressor lasso_(0.2, 1000, 0.001);
	lasso.fit(X, y);
	lasso_.fit(X, y);

	Matrix<dnn_double>& Y = lasso.predict(X_test);
	Matrix<dnn_double>& Y_ = lasso.predict(X_test);
	Y.print("lasso");
	(Y - Y_).print();
...
void test4()
{
	...
	ElasticNet_Regressor elasticNet_(0.2, 0.2, 1000, 0.001);
