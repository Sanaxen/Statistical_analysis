...
/* swilk.f -- translated by f2c (version 20090411).
...
class shapiro_wilk_
{
	...
	inline doublereal alnorm_(doublereal *x, logical *upper)
	{
		/* Initialized data */
	...
	inline doublereal ppnd_(real *p, integer *ifault)
	{
		/* Initialized data */
	...
	inline doublereal poly_(real *c__, integer *nord, real *x)
	{
		/* System generated locals */
		integer i__1;
		real ret_val;
	...
	inline doublereal pow_dd(doublereal *ap, doublereal *bp)
	{
		return(pow(*ap, *bp));
	}
...
public:
	/* Subroutine */ int wilk_(logical *init, real *x, integer *n, integer *n1,
		integer *n2, real *a, real *w, real *pw, integer *ifault)
	{
		...
		static real qtr = .25f;
		static real th = .375f;
		static real small_value = 1e-19f;
...
public:
	...
	inline dnn_double get_w() const
	{
		return w;
	}

	inline dnn_double p_value() const
	{
		return pw;
	}

	int test(Matrix<dnn_double>& x)
	{
		init = 0;
		integer ifault = 0;
		...
		{
			//printf("sample size %d > 5000\n", xr.size());
			std::mt19937 mt(0);
			std::uniform_real_distribution<> rand01(0.0, 1.0);

			double d = 1.0 - 5000.0 / (x.m*x.n)+ 0.01;
			for (int i = 0; i < x.m*x.n; i++)
			{
				if (rand01(mt) < d) continue;
				xr.push_back(x.v[i]);
				if (xr.size() == 5000) break;
			}
		}
		//printf("sample size = %d\n", xr.size());
		...
		pw = 999;
		ifault = -999;
		shapiro.wilk_(&init, x_, &n, &n1, &n2, a, &w, &pw, &ifault);
