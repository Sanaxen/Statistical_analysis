...
//				const size_t sz = μ.m*μ.n;
//#pragma omp parallel for
//				for (int i = 0; i < sz; i++)
//				{
//					μ.v[i] += a * rate_cf(engine) / (pow(accept,4.0) + 1);
//				}
...
#pragma omp parallel for
				for (int j = 0; j < xs.m; j++)
				{
					μ(j, var) += confounding_factors_zeros*a * rate_cf(engine) / (pow(accept, 4.0) + 1);
				}
			}
